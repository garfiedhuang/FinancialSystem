...
namespace Com.Garfield.Utility.common
{
    /// <summary>    
    /// 数据访问抽象基础类(ACCESS)    
    /// Copyright (C) 2017-2018 by garfield huang
    /// All rights reserved    
    /// </summary>    
    public abstract class AccessDbHelper
    {
        ...
        private static string GetConnectionString()
        {
            try
            {
                //var AAA = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=.\data\finance.accdb;Jet OLEDB:Database Password=5*5=30ya?;Persist Security Info=False";
                //var SSSS = SecurityHelper.DesEncrypt(AAA, SystemSetting.EncryptKey);

                var _connString = ConfigurationManager.ConnectionStrings["AccessConnectionString"].ConnectionString;
                if (_connString.Contains("ACE.OLEDB.12.0")) return _connString;
                return SecurityHelper.DesDecrypt(_connString, SystemSetting.EncryptKey);
            }
            finally { }
        }
        ...
        /// </summary>    
        /// <param name="SQLStringList">SQL语句的哈希表（key为sql语句，value是该语句的OleDbParameter[]）</param>    
        public static void ExecuteSqlTran(List<SQLStringList> SQLStringList)
        {
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                conn.Open();
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        foreach (var myDE in SQLStringList)
                        {
                            ...
                            OleDbParameter[] cmdParms = myDE.Parameters;
                            PrepareCommand(cmd, conn, trans, cmdText, cmdParms);
                            int val = cmd.ExecuteNonQuery();
                            cmd.Parameters.Clear();
                        }
        ...
        public static int GetRecordCount(string _ID, string _tbName, string _strCondition, int _Dist)
        {
            ...
            object obj = AccessDbHelper.GetSingle(sql);
            if (obj == null)
            {
                return 1;
            }
