...
namespace Com.Garfield.Utility
{
    public class SecurityHelper
    {
        ...
        private static readonly string Default_AES_Key = "@gbes.glodon.com";
        ...
        /// <returns></returns>
        public static bool AES_EncryptFile(string inputFile, string outputFile, string decryptKey)
        {
            try
            {
        ...
        /// <param name="decryptKey"></param>
        /// <returns></returns>
        public static bool AES_DecryptFile(string inputFile, string outputFile, string decryptKey)
        {
            try
            {
                FileStream fr = new FileStream(inputFile, FileMode.Open);
        ...
        public static void GetRSAKey(string path)
        {
            ...
            using (StreamWriter writer = new StreamWriter(Path.Combine(path, "RSA解密_PrivateKey_" + datetimestr + ".xml")))  //这个文件要保密...
            {
                writer.WriteLine(rsa.ToXmlString(true));
            }
            using (StreamWriter writer = new StreamWriter(Path.Combine(path, "RSA加密_PublicKey_" + datetimestr + ".xml")))
            {
        ...
        public static void RC4EncryptFile(string sourceFileName, string destinationFileName, string password)
        {
            try
            {
                var originalFile = GetBytes(sourceFileName);
                var encryptFile = EncryptRC4(originalFile, password);

                File.WriteAllBytes(destinationFileName, encryptFile);
            }
            finally { }
        }
        ...
        /// <param name="destinationFileName"></param>
        /// <param name="password"></param>
        public static void RC4DecryptFile(string sourceFileName, string destinationFileName, string password)
        {
            try
            {
                var encryptFile = GetBytes(sourceFileName);
                var decryptFile = DecryptRC4(encryptFile, password);

                File.WriteAllBytes(destinationFileName, decryptFile);
            }
            finally { }
        }
        ...
        /// <returns></returns>
        private static byte[] EncryptRC4(byte[] data, string pass)
        {
        ...
        /// <param name="pass"></param>
        /// <returns></returns>
        private static byte[] DecryptRC4(byte[] data, string pass)
        {
            return EncryptRC4(data, pass);
        }
